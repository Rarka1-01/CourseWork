...
public class MWolf extends Animal
{
    MWolf()
    {
        super();
    ...
    public int[] move(int i, int j, Animal[][] map)
    {
        ...
        ArrayList<int[]> posF = new ArrayList<>();
        ...
        for(int i1 = i-1; i1 < i+2; i1++)
            for(int j1 = j-1; j1 < j+2; j1++)
                if(i1 > -1 && i1 < 20 && j1 > -1 && j1 < 20 && (j1 != j || i1 != i))
                    ...
                    else if(map[i1][j1].getType() == 'F')
                        posF.add(new int[]{i1, j1});
        ...
        else if(!posF.isEmpty())
        {
            int o = (int)(Math.random() * posF.size());
            tmp[0] = posF.get(o)[0];
            tmp[1] = posF.get(o)[1];
        }
        else if(!pos.isEmpty())
